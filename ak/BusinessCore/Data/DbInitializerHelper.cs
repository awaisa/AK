...
namespace BusinessCore.Data
{
    public static class DbInitializerHelper
    {
        ...
        public static void Initialize(int? companyId = null)
        {
            try
            {
                using (_context = new ApplicationContext())
                {
                    ...
                    if (string.IsNullOrEmpty(_filename))
                        _filename = AppContext.BaseDirectory + "/App_Data/coa.csv";
                    ...
                    {
                        company = _context.Companies.FirstOrDefault(c=> c.Id == companyId);
                    }

                    if (_context.Users.Count() == 0)
                        DbInitializerHelper.InitialUserAndSecurityModel(company.Id);

                    FinancialYear fy = null;
                    if (_context.FiscalYears.Count() == 0)
                        fy = InitFiscalYear(company.Id);

                    List<PaymentTerm> paymentTerms = null;
                    if (_context.PaymentTerms.Count() == 0)
                        paymentTerms = InitPaymentTerms(company.Id);
                    ...
                    if (_context.Accounts.Count() == 0)
                    {
                        ...
                        throw new Exception("AK 04 May 2017: UpdateAccountsParentCodes commented, dont know why we need that method.");
                    }
                    ...
                    if (_context.Taxes.Count() == 0)
                        DbInitializerHelper.InitTax(company.Id);

                    List<Bank> banks = null;
                    if (_context.Banks.Count() == 0)
                        banks = InitBanks(company.Id);

                    Vendor vendor = null;
                    if (_context.Vendors.Count() == 0)
                        vendor = DbInitializerHelper.InitVendor(company.Id);

                    Customer customer = null;
                    if (_context.Customers.Count() == 0)
                        customer = DbInitializerHelper.InitCustomer(company.Id);

                    List<Item> items = null;
                    if (_context.Items.Count() == 0)
                        items = InitItems(company.Id);
                }
            }
            catch
            {
        ...
        public static void InitialUserAndSecurityModel(int companyId)
        {
            ...
            var user = new User() { Lastname = "System", Firstname = "Administrator", Username = "admin@email.com", CompanyId = companyId };
        ...
        public static FinancialYear InitFiscalYear(int companyId)
        {
            _context.FiscalYears.Add(new FinancialYear() { FiscalYearCode = "FY1516", FiscalYearName = "FY 2016/2017", StartDate = new DateTime(2016, 01, 01), EndDate = new DateTime(2016, 12, 31), IsActive = true, CompanyId = companyId });
            _context.SaveChanges();

            return _context.FiscalYears.FirstOrDefault();
        }

        public static List<PaymentTerm> InitPaymentTerms(int companyId)
        {
        ...
        public static List<Account> LoadChartOfAccountsFromFile(string filename, int companyId)
        {
            ...
            //{
                ...
                var importedAccounts = JsonConvert.DeserializeObject<ImportAccount[]>(json);
        ...
        public static void InitTax(int companyId)
        {
        ...
        public static Vendor InitVendor(int companyId)
        {
            Party vendorParty = new Party();
            vendorParty.Name = "ABC Sample Supplier";
            vendorParty.PartyType = BusinessCore.Domain.PartyTypes.Vendor;
            ...
            primaryContact.CompanyId = companyId;
        ...
        public static Customer InitCustomer(int companyId)
        {
            ...
            customerParty.PartyType = BusinessCore.Domain.PartyTypes.Customer;
        ...
        public static List<Item> InitItems(int companyId)
        {
        ...
        public static List<Bank> InitBanks(int companyId)
        {
            var bank = new Bank()
            {
