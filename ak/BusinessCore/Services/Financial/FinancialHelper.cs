...
namespace BusinessCore.Services.Financial
{
    public static class FinancialHelper
    {
        ...
        {
            decimal totalDebit = glHeader.GeneralLedgerLines.Where(d => d.DrCr == Domain.DrOrCrSide.Dr).Sum(d => d.Amount);
            decimal totalCredit = glHeader.GeneralLedgerLines.Where(d => d.DrCr == Domain.DrOrCrSide.Cr).Sum(d => d.Amount);
